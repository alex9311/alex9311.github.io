...
def get_person_detection_boxes(model, cv2_img, threshold=0.5):
    img = cv2.cvtColor(cv2_img, cv2.COLOR_BGR2RGB)  # Convert to RGB
    ...
    for idx, box in enumerate(pred_boxes):
        if pred_classes[idx] == 'person':
            cv2.rectangle(cv2_img, box[0], box[1], color=(0, 255, 0),
                          thickness=3)  # Draw Rectangle with the coordinates
            cv2.putText(cv2_img, pred_classes[idx], box[0], cv2.FONT_HERSHEY_SIMPLEX,
                        3, (0, 255, 0), thickness=3)  # Write the prediction class

    return cv2_img, pred_boxes
...
def main():
    ...
    while success:
        ...
        box_img, boxes = get_person_detection_boxes(box_model, image.copy(), threshold=0.8)
        cv2.imwrite(box_dir+'box%s.jpg' % count_str, box_img)
        box = boxes[0]  # assume there is only 1 person
        center, scale = box_to_center_scale(box, cfg.MODEL.IMAGE_SIZE[0], cfg.MODEL.IMAGE_SIZE[1])

        # pose estimation
        preds = get_pose_estimation_prediction(pose_model, image, center, scale)
